    ...
    <div class="modal-content">
      <form id="createForm" data-parsley-validate="" enctype="multipart/form-data"> 
        ...
        <div class="modal-header">
          <h5 id="exampleModalLabel">Create Material</h5>
        ...
        <div class="modal-body">
          <div class="m-scrollable" data-scrollbar-shown="true" data-scrollable="true" data-height="400"> 
            <div class="form-group m-form__group row">
              <div class="col-md-12">
                <label>Type</label>
                 <select  class="form-control m-input" name="materialType" required>
                  <?php $result=db_select_fillselect('tbl_selectmo','materialType');
                ...
                <textarea name="itemDescription" class="form-control" data-provide="markdown" rows="10" required></textarea>
              </div>
                <div class="col-md-4">
                <label>Quantity</label>
                <input type="number" name="quantity" class="form-control m-input"   required min="1" value ="1" />
              </div>
                <div class="col-md-4">
                <label>Unit</label>
                 <select  class="form-control m-input" name="unit" required>
                  <?php $result=db_select_fillselect('tbl_unit','unit');
                  for($i=1; $row = $result->fetch(); $i++){ ?>
                ...
                <div class="col-md-4">
                <label>Price</label>
                <input type="number" name="price" class="form-control m-input"  placeholder="Enter Price"  required  min="1" />
              </div>
            </div>
...
<?php
$sql = custom_query("SELECT * FROM tbl_materials ORDER BY id DESC");
if($sql->rowCount()>0) {
  while ($r = $sql->fetch(PDO::FETCH_ASSOC))
    {   ?> 
      <div class="modal fade" id="reorder<?= $r['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
          <div class="modal-content">
            <form class="deleteForm" data-parsley-validate="">
              ...
              <div class="modal-header">
                <h5 id="exampleModalLabel">Reorder Material Record</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <h3>Are you sure you want to reorder this record?</h3>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal"><span><i class="la la-times"></i></span> <span>Cancel</span></button>
                <button type="submit" class="btn btn-danger"><span><i class="la la-trash"></i> </span> <span>reorder</span></button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <form class="updateForm" data-parsley-validate="" enctype="multipart/form-data"> 
            ...
            <input type="hidden" name="id" value="<?= $r['id']; ?>" />
            <div class="modal-header">
              <h5 id="exampleModalLabel">Edit Material Record</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
    ...
    <!--begin::Page Scripts --> 
    <script src="app/materials.js" type="text/javascript"></script>
    <!--end::Page Scripts -->
